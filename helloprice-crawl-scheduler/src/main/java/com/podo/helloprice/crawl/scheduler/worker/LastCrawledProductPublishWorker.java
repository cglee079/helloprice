...
import com.podo.helloprice.crawl.scheduler.service.LastCrawledProductService;
...
@Component
public class LastCrawledProductPublishWorker implements Worker {

    @Value("${product.publish.expire.minute}")
    private Integer crawlExpireMinute;

    private final EmitterProcessor<String> processor;
    private final LastCrawledProductService lastCrawledProductService;

    @Override
    public void run(LocalDateTime now) {
        final LastPublishedProduct lastPublishedProduct = lastCrawledProductService.getLastCrawledProduct(now.minusMinutes(crawlExpireMinute));
    ...
    private void publish(LastPublishedProduct lastPublishedProduct, LocalDateTime lastPublishAt) {
        try {
            ...
            lastCrawledProductService.updateLastPublishAt(lastPublishedProduct.getProductCode(), lastPublishAt);
