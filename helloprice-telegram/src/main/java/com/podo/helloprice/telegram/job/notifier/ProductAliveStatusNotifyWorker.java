...
public class ProductAliveStatusNotifyWorker implements Worker {
    ...
    public void doIt(LocalDateTime now) {
        ...
        handleProductUpdateStatusUpdated(now);
        handleProductAliveStatusDead();
    }

    private void handleProductUpdateStatusUpdated(LocalDateTime notifyAt) {
        final List<ProductDetailDto> products = new ArrayList<>();

        products.addAll(productReadService.findByStatusAndUpdateStatus(ProductAliveStatus.ALIVE, ProductUpdateStatus.UPDATED));
        products.addAll(productReadService.findByStatusAndUpdateStatus(ProductAliveStatus.PAUSE, ProductUpdateStatus.UPDATED));

        for (ProductDetailDto product : products) {
            handleByProductSaleStatus(notifyAt, product);
            productWriteService.notifiedProduct(product.getId());
        }
    ...
    private void handleProductAliveStatusDead() {
        List<ProductDetailDto> products = productReadService.findByStatusAndUpdateStatus(ProductAliveStatus.DEAD, ProductUpdateStatus.UPDATED);
