package com.podo.helloprice.telegram.app.menu.emailadd;
...
import com.podo.helloprice.telegram.app.menu.emailkey.EmailKeyResponse;
...
public class EmailAddMenuHandler extends AbstractMenuHandler {
    ...
    public void handle(MessageVo messageVo, String requestMessage) {
        ...
        if (!EmailValidator.getInstance().isValid(email)) {
            ...
            sender().send(SendMessageVo.create(messageVo, EmailAddResponse.invalidEmail(), Keyboard.getHomeKeyboard(productCommands), callbackFactory.create(telegramId, HOME)));
            return;
        }
    ...
    private void handleEmailAdd(MessageVo messageVo, String email) {
        ...
        sender().send(SendMessageVo.create(messageVo, EmailKeyResponse.explain(), Keyboard.getDefaultKeyboard(), callbackFactory.create(telegramId, EMAIL_KEY)));
    }
