...
public class TelegramMessageReceiver {
    ...
    private void sendMessageToNewUser(TMessageVo tMessageVo) {
        final String telegramId = tMessageVo.getTelegramId();
        telegramMessageSender.send(tMessageVo.newMessage(CommonResponse.introduce(appName), Keyboard.getHomeKeyboard(Collections.emptyList()), callbackFactory.create(telegramId, Menu.HOME)));
        telegramMessageSender.sendWithWebPagePreview(tMessageVo.newMessage(CommonResponse.help(helpUrl), null, callbackFactory.create(telegramId, null)));
        telegramMessageSender.send(tMessageVo.newMessage(CommonResponse.seeKeyboardIcon(), null, callbackFactory.create(telegramId, null)));
    }
