...
public class ItemDeleteMenuHandler extends AbstractMenuHandler {
    ...
    public void handle(TMessageVo tMessageVo, String requestMessage) {
        ...
        if (!userItemNotifyService.isExistedNotify(userDetail.getId(), itemDetail.getId())) {
            ...
            sender().send(tMessageVo.newMessage(ItemDeleteResponse.alreadyNotNotifyItem(), km.getHomeKeyboard(itemCommands), callbackFactory.createDefault(telegramId, Menu.HOME)));
        }
        ...
        sender().send(tMessageVo.newMessage(ItemDeleteResponse.deletedNotifyItem(itemDetail), km.getHomeKeyboard(reloadItemCommands), callbackFactory.createDefault(telegramId, Menu.HOME)));
