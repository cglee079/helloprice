...
class CrawlProductUpdateEventJobWriterTest {
    ...
    void testFailCrawl() {
        ...
        given(product.increaseDeadCount(any())).willReturn(true);
        ...
        //given - mock productSaleRepository
        final ProductSaleRepository productSaleRepository = Mockito.mock(ProductSaleRepository.class);

        //given -
        final ProductUpdateEventStore productUpdateEventStore = new ProductUpdateEventStore();

        //when
        final CrawlProductUpdateJobWriter crawlProductUpdateJobWriter =
                new CrawlProductUpdateJobWriter(
                        crawlProductJobParameter,
                        productUpdateEventStore,
                        productRepository,
        ...
        //then
        then(product).should(times(1)).increaseDeadCount(any());
